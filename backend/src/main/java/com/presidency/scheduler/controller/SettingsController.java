...
public class SettingsController {
    ...
    public ResponseEntity<?> getHolidays() {
        return ResponseEntity.ok(ApiResponse.ok(holidayRepo.findAll()));
    }
    ...
    public ResponseEntity<?> addHoliday(@RequestBody Map<String, String> body) {
        try {
            ...
            if (holidayRepo.existsByDate(date))
                return ResponseEntity.badRequest().body(ApiResponse.error("Holiday already exists"));
            return ResponseEntity.ok(
                    ApiResponse.ok(holidayRepo.save(Holiday.builder().date(date).label(body.get("label")).build())));
        } catch (Exception e) {
    ...
    public ResponseEntity<?> getBlocked() {
        return ResponseEntity.ok(ApiResponse.ok(blockedRepo.findAll()));
    }

    @PostMapping("/blocked-slots")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<?> addBlocked(@RequestBody Map<String, String> body) {
        try {
            var day = TeacherAvailability.Day.valueOf(body.get("day"));
            if (blockedRepo.existsByDayAndTimeSlot(day, body.get("timeSlot")))
                return ResponseEntity.badRequest().body(ApiResponse.error("Slot already blocked"));
            return ResponseEntity.ok(ApiResponse.ok(blockedRepo.save(
                    BlockedSlot.builder().day(day).timeSlot(body.get("timeSlot")).reason(body.get("reason")).build())));
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(ApiResponse.error(e.getMessage()));
        }
    }
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<?> deleteBlocked(@PathVariable Long id) {
